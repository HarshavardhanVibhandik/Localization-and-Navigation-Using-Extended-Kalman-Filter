...
for i = 1:length(sampledTime) % This is For loop that iterates over each time step in the sampledTime
    ...
    % The Prediction step
    [covarEst, uEst] = pred_step(uPrev, covarPrev, angVel, acc, dt); %The predicion state of the Kalman filter

    % The Update step
    [uCurr, covar_curr] = upd_step(z_t,covarEst,uEst); % Updates the state and covariance based on the measurement data and the predicted state and covariance
